...
RED_CAPTURE_SOURCES=$(wildcard $(SRCDIR)/$(RED_CAPTURE_DIR)/*.cpp)
RED_CAPTURE_OBJECTS=$(patsubst $(SRCDIR)/$(RED_CAPTURE_DIR)/%.cpp, $(OBJDIR)/$(RED_CAPTURE_DIR)/%.o, $(RED_CAPTURE_SOURCES))
RED_CAPTURE_INCLUDES=$(wildcard $(INCDIR)/$(RED_CAPTURE_DIR)/*.h)

EXECUTABLES=camera_capture red_capture red_finder
...
$(RED_CAPTURE_OBJECTS): $(OBJDIR)/$(RED_CAPTURE_DIR)/%.o: $(SRCDIR)/$(RED_CAPTURE_DIR)/%.cpp $(RED_CAPTURE_INCLUDES)
	$(CC) $(CFLAGS) $< -o $@ $(LIBS)
